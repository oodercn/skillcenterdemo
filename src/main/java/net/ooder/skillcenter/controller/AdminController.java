...
public class AdminController {
    ...
    public ResponseEntity<ApiResponse<List<Map<String, Object>>>> getAllSkills(
        ...
        // 转换为包含所有字段的DTO列表
        List<Map<String, Object>> skillDTOs = skills.stream()
            .filter(skill -> {
                // 分类筛选
                if (category != null && !category.isEmpty()) {
                    String skillCategory = getSkillCategory(skill);
                ...
                if (status != null && !status.isEmpty()) {
                    String skillStatus = getSkillStatus(skill);
                    if (!status.equals(skillStatus)) {
                        return false;
                    }
                }
                // 关键词筛选
                if (keyword != null && !keyword.isEmpty()) {
                    ...
                    String skillId = skill.getId() != null ? skill.getId().toLowerCase() : "";
                    String skillName = skill.getName() != null ? skill.getName().toLowerCase() : "";
                    if (!skillId.contains(lowerKeyword) && !skillName.contains(lowerKeyword)) {
                        return false;
                    }
                }
                return true;
            ...
            .map(skill -> {
                ...
                dto.put("category", getSkillCategory(skill));
                dto.put("status", getSkillStatus(skill));
                dto.put("createdAt", getSkillCreatedAt(skill));
                dto.put("available", skill.isAvailable());
                return dto;
            })
    ...
    private String getSkillCategory(Skill skill) {
        if (skill instanceof SkillManager.SkillInfo) {
            return ((SkillManager.SkillInfo) skill).getCategory();
        }
        return "general";
    }
     ...
     * 获取技能状态
     */
    private String getSkillStatus(Skill skill) {
        if (skill instanceof SkillManager.SkillInfo) {
            return ((SkillManager.SkillInfo) skill).getStatus();
        }
        return skill.isAvailable() ? "active" : "inactive";
    }
    
    /**
     * 获取技能创建时间
     */
    private String getSkillCreatedAt(Skill skill) {
        if (skill instanceof SkillManager.SkillInfo) {
            Date createdAt = ((SkillManager.SkillInfo) skill).getCreatedAt();
            if (createdAt != null) {
     ...
     */
    @PostMapping("/skills")
    public ResponseEntity<ApiResponse<Skill>> addSkill(@RequestBody SkillManager.SkillInfo skillInfo) {
        try {
            Skill newSkill = skillManager.addSkill(skillInfo);
     ...
     */
    @GetMapping("/users")
    public ResponseEntity<ApiResponse<List<UserManager.User>>> getAllUsers() {
        try {
    ...
    public ResponseEntity<ApiResponse<UserManager.User>> getUser(@PathVariable String userId) {
        try {
            ...
            return ResponseEntity.status(500)
                    .body(ApiResponse.error(500, "获取用户失败: " + e.getMessage()));
        }
    }
     ...
     */
    @PostMapping("/users")
    public ResponseEntity<ApiResponse<UserManager.User>> createUser(@RequestBody UserManager.User user) {
        try {
            UserManager.User createdUser = userManager.createUser(user);
    ...
    public ResponseEntity<ApiResponse<UserManager.User>> updateUser(@PathVariable String userId, @RequestBody UserManager.User user) {
        try {
            user.setId(userId);
            UserManager.User updatedUser = userManager.updateUser(user);
    ...
    public ResponseEntity<ApiResponse<List<UserManager.User>>> searchUsers(@RequestParam String keyword) {
        try {
            List<UserManager.User> users = userManager.searchUsers(keyword);
            return ResponseEntity.ok(ApiResponse.success(users));
        } catch (Exception e) {
            return ResponseEntity.status(500)
    ...
    public ResponseEntity<ApiResponse<List<UserManager.User>>> getGroupMembers(@PathVariable String groupId) {
        try {
            List<UserManager.User> users = userManager.getUsersByGroupId(groupId);
            return ResponseEntity.ok(ApiResponse.success(users));
        } catch (Exception e) {
            return ResponseEntity.status(500)
                    .body(ApiResponse.error(500, "获取群组成员失败: " + e.getMessage()));
        }
    }
