...
window.submitCreate = async function() {
    ...
    try {
        const result = await utils.api.post('/api/scene-groups/create', { sceneId: sceneId, config: config });
...
window.viewMembers = async function(sceneGroupId) {
    ...
    try {
        const result = await utils.api.post('/api/scene-groups/members', { 
...
window.generateKey = async function(sceneGroupId) {
    ...
    try {
        const result = await utils.api.post('/api/scene-groups/key/generate', { sceneGroupId: sceneGroupId });
        if (result.code === 200 && result.data) {
...
window.destroyGroup = async function(sceneGroupId) {
    ...
    try {
        const result = await utils.api.post('/api/scene-groups/destroy', { sceneGroupId: sceneGroupId });
        if (result.code === 200 && result.data) {
            if (sceneGroupTable) sceneGroupTable.refresh();
