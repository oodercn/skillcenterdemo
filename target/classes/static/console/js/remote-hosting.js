...
function initHostingTable() {
    ...
    hostingTable = utils.DataTable({
        ...
        apiUrl: '/admin/hosting/instances',
        ...
        columns: [
            ...
            { field: 'uptime', title: '运行时长', width: '100px', align: 'center' },
            {
                field: 'actions',
                title: '操作',
                width: '300px',
...
// 打开编辑托管模态框
window.editHosting = async function(hostingId) {
    console.log('[RemoteHosting] 编辑托管实例:', hostingId);

    try {
        const data = await utils.api.get(`/admin/hosting/instances/${hostingId}`);
...
// 启动托管实例
window.startHosting = async function(hostingId) {
    console.log('[RemoteHosting] 启动托管实例:', hostingId);
    try {
        const result = await utils.api.post(`/admin/hosting/instances/${hostingId}/start`);
...
// 停止托管实例
window.stopHosting = async function(hostingId) {
    console.log('[RemoteHosting] 停止托管实例:', hostingId);
    try {
        const result = await utils.api.post(`/admin/hosting/instances/${hostingId}/stop`);
...
// 删除托管实例
window.deleteHosting = async function(hostingId) {
    if (!utils.msg.confirm('确定要删除这个托管实例吗？')) {
        return;
    }

    console.log('[RemoteHosting] 删除托管实例:', hostingId);
    try {
        const result = await utils.api.delete(`/admin/hosting/instances/${hostingId}`);
...
window.submitHostingForm = async function() {
    ...
    try {
        let result;
        if (hostingId) {
            console.log('[RemoteHosting] 更新托管实例:', hostingId);
            result = await utils.api.put(`/admin/hosting/instances/${hostingId}`, hostingData);
        } else {
            console.log('[RemoteHosting] 创建托管实例');
            result = await utils.api.post('/admin/hosting/instances', hostingData);
        }
